...
surprises when your content is complex. Here is a quick example of
how content processing events are ordered:
...
method returns the most recebt output text generated by `object.do()`
Then print knows enough to use that when the object is thrown in its
face, so to speak. But not everything in Python is so smart. For
